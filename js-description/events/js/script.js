...
document.addEventListener("click", function (e) {
	...
	if (target.closest('.block__button-sub_active')) {
		colapseInput(target);
	} else if (target.closest('.block__button-sub') || target.closest('.block__button-close')) {
		colapseInput(target);
		clearInput();
		e.preventDefault();
	}
...
document.addEventListener("keyup", function (e) {
	if (e.key == 'Escape') {
		const buttonSub = document.querySelector('.block__button-sub');
		if (buttonSub.closest('.block__button-sub_active')) {
			colapseInput();
			clearInput();
		}
	}
	const formInput = document.querySelector('.block__input-text');
	const formLabel = document.querySelector('.block__input-label');
	quantity = 30 - formInput.value.length;
	if (quantity == 30) {
		formLabel.innerHTML = '';
	} else {
...
function colapseInput(e) {
	const formInput = document.querySelector('.block__input-text');
	formInput.classList.toggle('block__input-text_disabled');
	const formButtonSub = document.querySelector('.block__button-sub');
	formButtonSub.classList.toggle('block__button-sub_active');
	const formButtonClose = document.querySelector('.block__button-close')
	formButtonClose.classList.toggle('block__button-close_disabled');
}
